namespace MVC.Controllers
{
    ...
    [RoutePrefix("api/wbinfo")]
    public class WbInfoController : ApiController
    {
        ...
        [HttpGet, AllowAnonymous, Route("tabs")]
        public async Task<HttpResponseMessage> GetAllPreferences(string wb)
        {
            ...
            try
            {
                ...
                List<WorkbookTab> allWbPreferences = await manager.GetTabDetailAsync(wb);

                ApiTabDetailsResponse response = new ApiTabDetailsResponse { IsError = false };
                
                if (allWbPreferences != null)
                {
                    ApiTabDetails info = new ApiTabDetails();
                    info.Workbook = wb;
                    info.TabList = allWbPreferences;
                    response.Content.Add(info);
                    response.StatusCode = HttpStatusCode.OK;
                }   

                return Request.CreateResponse<ApiTabDetailsResponse>(HttpStatusCode.OK, response);
            }
            catch(Exception ex)
            {
        ...
        [HttpPost, AllowAnonymous, Route("apis")]
